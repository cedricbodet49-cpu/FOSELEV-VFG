(() => {
  ...
  function syncTyreSection(visit = state.activeVisit) {
    ...
    section.points = visit.tyres.axles.flatMap((axle, axleIndex) => axle.tyres.map(tyre => {
      ...
      return {
        ...
        status: tyreValueToPointStatus(tyre.value),
        findingId: tyre.findingId || old.findingId || null,
  ...
  function renderTyreInspection() {
    ...
    $('#tyreProgress').textContent = `${progress.remaining}/${progress.total}`;
    $('#axleCountSelector').innerHTML = Array.from({length:8},(_,i)=>i+2).map(count => `<button type="button" class="axle-count-button${count===data.axleCount?' is-active':''}" data-axle-count="${count}">${count}</button>`).join('');
    ...
    diagram.innerHTML = data.axles.map((axle, axleIndex) => {
      const tyreButton = tyre => {
        ...
        return `<button type="button" class="tyre-wheel tyre-${String(tyre.value).toLowerCase()}" data-tyre-id="${tyre.id}" aria-label="${escapeHtml(tyrePositionLabel(axleIndex, tyre.side, tyre.position))} : ${label}"><span>${label}</span></button>`;
      };
      ...
      return `<section class="tyre-axle-row">
        ...
        <button type="button" class="tyre-axle-label" data-toggle-axle="${axleIndex}"><strong>ESSIEU ${axleIndex+1}</strong><span>${axle.mode === 'dual' ? 'Roues jumelées' : 'Roues simples'}</span></button>
  ...
  function openTyreStateDialog(pointId) {
    ...
    $('#tyreDialogTitle').textContent = point?.label || 'Pneumatique';
    $('#tyreStateOptions').innerHTML = renderTyreStatusButtons(tyre.value);
    $('#tyreCreateFinding').classList.toggle('hidden', tyre.value !== 'HS');
    $('#tyreStateDialog').classList.remove('hidden');
    $('#tyreStateDialog').setAttribute('aria-hidden','false');
  }

  function closeTyreStateDialog() {
    $('#tyreStateDialog').classList.add('hidden');
    $('#tyreStateDialog').setAttribute('aria-hidden','true');
  }
  ...
  async function deleteFinding() {
    ...
    point.findingId = null;
    const tyre = findTyre(point.id);
    if (tyre) { tyre.findingId = null; point.status = tyreValueToPointStatus(tyre.value); }
    else point.status = 'pending';
    state.draftPhotos = [];
    state.draftFindingId = null;
    state.originalPhotoIds = [];
    state.removedPhotoIds = [];
  ...
  function buildReportPreview() {
    ...
    const cover = `
      ...
      <div class="report-cover-summary">
        ...
        <div><strong>${findings.length + tyreHs.length + tyreWatch.length}</strong><span>anomalies / surveillances</span></div>
      </div>`;

    const automaticTyreRows = [
      ...tyreHs.map(item => ({ level: 'ANOMALIE CRITIQUE / MAJEURE', title: item.label, comment: 'Pneumatique HS' })),
      ...tyreWatch.map(item => ({ level: 'À SURVEILLER', title: item.label, comment: 'Usure restante : 25 %' }))
    ];
    const allSummaryItems = [
      ...findings.map(finding => ({ level: finding.level || 'Anomalie', title: finding.pointLabel || finding.title || 'Point de contrôle', comment: finding.comment || 'Aucun commentaire.' })),
      ...automaticTyreRows
    ];
    const summaryRows = allSummaryItems.length ? allSummaryItems.map((finding, index) => `
      <div class="report-anomaly-row">
        <span class="report-anomaly-number">${index + 1}</span>
        <div><strong>${escapeHtml(finding.level)}</strong><span>${escapeHtml(finding.title)}</span></div>
    ...
    function zoneContent(zone, number, title) {
      ...
      const blocks = sections.map(section => {
        if (zone === 'carrier' && section.id === 'tyres') {
          const axleHtml = visit.tyres.axles.map((axle, axleIndex) => `<div class="report-tyre-axle"><strong>Essieu ${axleIndex + 1}</strong><span>${axle.mode === 'dual' ? 'Roues jumelées' : 'Roues simples'}</span><div>${axle.tyres.map(tyre => `<span class="report-tyre report-tyre-${String(tyre.value).toLowerCase()}">${escapeHtml(tyrePositionLabel(axleIndex, tyre.side, tyre.position))}<b>${tyre.value === 'pending' ? 'NON CONTRÔLÉ' : tyre.value === 'HS' ? 'HS' : tyre.value + ' %'}</b></span>`).join('')}</div></div>`).join('');
          return `<section class="report-control-section report-tyres"><h3>Pneumatiques</h3>${axleHtml}</section>`;
        }
  ...
  function bindEvents() {
    ...
    $('#tyreDiagram').addEventListener('click', event => {
      ...
      else if (tyre) openTyreStateDialog(tyre.dataset.tyreId);
    });
