  ...
  function renderTyreInspection() {
    ...
    diagram.innerHTML = data.axles.map((axle, axleIndex) => {
      const tyreButton = tyre => {
        ...
        return `<button type="button" class="tyre-wheel tyre-${String(tyre.value).toLowerCase()}${nextClass}" data-tyre-id="${tyre.id}" aria-label="${escapeHtml(tyrePositionLabel(axleIndex, tyre.side, tyre.position))} : ${label}"><span>${label}</span></button>`;
      };
